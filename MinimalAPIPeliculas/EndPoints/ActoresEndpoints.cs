...
namespace MinimalAPIPeliculas.EndPoints
{
    ...
    {
        private static readonly string contenedor = "actores"; // Carpeta donde se van a almacenar

        public static RouteGroupBuilder MapActores(this RouteGroupBuilder group)
        {
            group.MapPost("/", Crear).DisableAntiforgery().AddEndpointFilter<FiltroValidaciones<CrearActorDTO>>().RequireAuthorization("esadmin"); ; ; // Evita recibir archivos en una app.
        ...
        static async Task<Results<Created<ActorDTO>, ValidationProblem>> Crear(
        ...
        static async Task<Ok<List<ActorDTO>>> ObtenerTodos(
            IRepositorioActores repositorio,
            IMapper mapper,
            int pagina = 1,
            int recordsPorPagina = 10)
        {
            var paginacion = new PaginacionDTO { Pagina = pagina, RecordsPorPagina = recordsPorPagina }; // Paginacion
        ...
        static async Task<Ok<List<ActorDTO>>> ObtenerPorNombre(
            string nombre,
            IRepositorioActores repositorio,
            IMapper mapper)
        {
        ...
        static async Task<Results<Ok<ActorDTO>, NotFound>> ObtenerPorId(
            int id,
            IRepositorioActores repositorio,
            IMapper mapper)
        {
